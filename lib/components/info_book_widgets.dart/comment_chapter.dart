...
class CommentChapter extends StatefulWidget {
  ...
  CommentChapter({super.key, required this.idBook, required this.chapterIndex});

  @override
  State<CommentChapter> createState() => _CommentChapterState();
}
...
class _CommentChapterState extends State<CommentChapter> {
  ...
  void initState() {
    ...
    if (user != null) {
      ...
      if (user.email != null) {
        ...
        // Fallback if no email or phone, use UID
        getNameUser(user.uid);
      }
  ...
  void sendComment() async {
    if (commentController.text.isNotEmpty && uid != null && userName != null) {
      print('object1');
      if (rootCommentID != null) {
        ...
        await FirebaseFirestore.instance // cập nhập hàm reply
            .collection('books')
            .doc(widget.idBook)
            .collection('chapter_comment')
            .doc('${widget.chapterIndex}')
